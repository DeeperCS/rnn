clc; clear; close all;
% Implementation of RNN based on lectures by Prof. Zhang
...
for tt = 1:trainLength
    ...
    p1 = zeros(n, size(w_1, 1), size(w_1, 2));
    p2 = zeros(n, size(w_2, 1), size(w_2, 2));
...
%             for di = 1:n
%                 for dj = 1:(n+m)
%                     p1(c, di, dj) = p1(c, di, dj) + df1(net_1(di)) * f2(net_2(di)) * ( delta(c, di) * y_u(dj) );
% 
%                     p2(c, di, dj) = p2(c, di, dj) + f1(net_1(di)) * df2(net_2(di)) * ( delta(c, di) * y_u(dj) );
%                 end
%             end
%         end
% 
%         for c = 1:n
%            gW1 = gW1 + squeeze(p1(c, :, :)) * dSc(c);
%            gW2 = gW2 + squeeze(p2(c, :, :)) * dSc(c);
%         end
