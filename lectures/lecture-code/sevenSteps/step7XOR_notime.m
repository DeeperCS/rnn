...
for t = 2:T
    ...
    dOut = dSc .* h(s_c) .* df_out(net_out);
    dSc = dSc .* dh(s_c) .* f_out(net_out);
    
    % compute the gradient of w_out
    for di = 1:size(w_out, 1)
        for dj = 1:size(w_out, 2)
            gWout(di, dj) = dOut(di) * y_u(dj);
        end
    end
    ...
    for di = 1:size(gW1, 1)
        for dj = 1:size(gW1, 2)
            gW1(di, dj) = p(1, di, dj) * dSc(di);
            gW2(di, dj) = p(2, di, dj) * dSc(di);
        end
    end
