...
for tt = 1:trainLength
    ...
    for t = size(x, 1):-1:2
        ...
        gW_R = gW_R + ( delta_s{t} .* df(net{t}) ) * [s{t-1}; ones([size(s{t-1}, 2), 1])]';
        gW_I = gW_I + ( delta_s{t} .* df(net{t}) ) * [x(t-1, :), ones([size(x(t-1, :), 1), 1])];
        
        % delta_s{t-1} = delta_s{t} .* ( 1 + cos(t/T)^2 .* (df(net{t}) - 1) );
        delta_s{t-1} = W_R(:, 1:end-1)' * (delta_s{t} .* df(net{t}));
