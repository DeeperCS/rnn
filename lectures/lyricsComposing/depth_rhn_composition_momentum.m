...
% Momentum
v_W_in_T = zeros(size(W_in_T));
v_W_in_H = zeros(size(W_in_H));
for layer =2:depth
    v_W_R_T{layer} = normrnd(0, 0.1, [n, n+1]);
    v_W_R_H{layer} = normrnd(0, 0.1, [n, n+1]);
end
v_W_kc = zeros(size(W_kc));
...
%             fprintf('t:%d, y:[%f, %f] net_out:[%f, %f] %f \n', t, min(y{t}), max(y{t}), min(net_out), max(net_out),  err );
        ...
        % Update (Momentum)
        v_W_in_T = mu * v_W_in_T - alpha * gW_in_H;
        v_W_in_H = mu * v_W_in_T - alpha * gW_in_T;
        for layer =2:depth
            v_W_R_T{layer} = mu * v_W_R_T{layer} - alpha * gW_R_T{layer};
            v_W_R_H{layer} = mu * v_W_R_T{layer} - alpha * gW_R_H{layer};
        end
        v_W_kc = mu * v_W_kc - alpha * gW_kc;

        W_in_H = W_in_H + v_W_in_T;
        W_in_T = W_in_T + v_W_in_H;
        for layer = 2:depth
            W_R_T{layer} = W_R_T{layer} + v_W_R_T{layer};
            W_R_H{layer} = W_R_H{layer} + v_W_R_H{layer};
        end
        W_kc = W_kc + v_W_kc;
