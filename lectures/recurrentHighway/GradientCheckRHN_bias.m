...
%     input = input(1:3, :);
    ...
    for t = 2:size(x, 1)
        ...
        net_T{t} = W_in_T * [x(t-1, :), ones([size(x(t-1, :), 1), 1])]' + W_R_T * [s{t-1}; ones([size(s{t-1}, 2)], 1)];
        s{t} = f_H(net_H{t}) .* f_T(net_T{t}) + s{t-1} .* (1 - f_T(net_T{t}));
        net_k{t} = W_kc * [s{t}; ones([size(s{t-1}, 2)], 1)];
        y{t} = f_k(net_k{t});
        y_k = y{t};
        err = 0.5 * (y_k - d)' * (y_k - d);
        errMean = [errMean; err];
    ...
    for t = size(x, 1):-1:2
        ...
        gW_kc = gW_kc + delta_K * [s{t}; ones([size(s{t-1}, 2)], 1)]' ;
        
        % backpropagation to s
        delta_s = (W_kc(:, 1:end-1)' * delta_K) + delta_next;

        gW_in_T = gW_in_T + ( delta_s .* ( df_T(net_T{t}) .* f_H(net_H{t}) - s{t-1} .* df_T(net_T{t}) ) ) * [x(t-1, :), ones([size(x(t-1, :), 1), 1])];
        gW_in_H = gW_in_H + ( delta_s .* f_T(net_T{t}) .* df_H(net_H{t}) ) * [x(t-1, :), ones([size(x(t-1, :), 1), 1])];
        gW_R_T = gW_R_T + ( delta_s .* ( df_T(net_T{t}) .* f_H(net_H{t}) - s{t-1} .* df_T(net_T{t}) ) ) * [s{t-1}; ones([size(s{t-1}, 2)], 1)]' ;
        gW_R_H = gW_R_H + ( delta_s .* f_T(net_T{t}) .* df_H(net_H{t}) ) * [s{t-1}; ones([size(s{t-1}, 2)], 1)]' ;
        % backpropagation through time
        delta_next = ...
            W_R_H(:, 1:end-1)' * (delta_s .* df_H(net_H{t}) .* f_T(net_T{t}))  +  ...
            W_R_T(:, 1:end-1)' * (delta_s .* f_H(net_H{t}) .* df_T(net_T{t})) + ...
            delta_s .* (1 - f_T(net_T{t})) - ...
            W_R_T(:, 1:end-1)' * (delta_s .* s{t-1} .* df_T(net_T{t}));
    end
